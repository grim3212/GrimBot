...
import { Presence, Game, GuildMember } from 'discord.js';
        ...
        if (await guildSettings.get(Constants.useWaitTimeName)) {
          ...
          if (await guildStorage.exists(Constants.lastNotificationName + '.' + newMember.id)) {
            ...
            if ((currentTime - oldTime) >= waitTime) {
              await this._tryShowGameNotification(oldMember, newMember, true);
            } else {
          ...
          await this._tryShowGameNotification(oldMember, newMember, false);
        }
      }
    }
  }

  private async _tryShowGameNotification(oldMember: GuildMember, newMember: GuildMember, waitTime: boolean): Promise<void> {
    ...
    if (sending) {
      ...
      console.log(message.replace('<game>', `${newMember.presence.game.name}`).replace('<user>', `${newMember.displayName}`));
